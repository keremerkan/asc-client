...
Özet, birimleri başlığa ve **ürün türü tanımlayıcısına** göre gruplar — bu yararlıdır çünkü tek bir rapor; ilk kez indirmeleri (`1*`/`3*`), güncellemeleri (`7*`) ve uygulama içi satın almaları (`IA*`) bir arada içerir. Tüm veriler için `--raw` kullanın.
...
- `--processing-date` — indirilecek örnek (`YYYY-MM-DD`); varsayılan olarak en günceli.
- `--ongoing` — tek seferlik anlık görüntü yerine sürekli bir rapor isteği kullanır.
- `--output` — indirilen bölüm dosyaları için dizin (varsayılan `./<app>-analytics`).
...
:::info Puanlar
App Store Connect API, toplam puan sayılarını veya yıldız puanı ortalamasını/histogramını sunmaz — yalnızca tek tek değerlendirmeleri (bkz. [Müşteri Değerlendirmeleri](./reviews.md)). İndirme ve gelir rakamları için yukarıdaki raporları kullanın.
:::
